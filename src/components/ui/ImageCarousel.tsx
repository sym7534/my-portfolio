...
function getSpans(w: number, h: number): Spans {
  const r = w / h;
  if (r > 2.2) return { colSpan: 3, rowSpan: 1 };
  if (r > 1.3) return { colSpan: 2, rowSpan: 1 };
...
export function ImageCarousel({ images }: ImageCarouselProps) {
  ...
  const handleLoad = useCallback((idx: number, e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    setAspectMap(prev => ({ ...prev, [idx]: getSpans(img.naturalWidth, img.naturalHeight) }));
  }, []);
  ...
  const handleMouseEnter = useCallback((idx: number) => {
    ...
    if (el) {
      ...
      if (!isNaN(col) && !isNaN(row)) {
        const clampedCol = Math.min(col, GRID_COLS - HOVER_COL_SPAN + 1);
      ...
      <AnimatePresence>
        {selectedIdx !== null && selectedImg && (
          <>
            ...
            <div
              className="fixed inset-0 z-[201] flex items-center justify-center pointer-events-none p-8"
            >
              <motion.img
                key={selectedIdx}
                src={selectedImg.src}
